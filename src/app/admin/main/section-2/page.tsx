...
export default function Section2Admin() {
  const { sections, updateSection } = useSections()
  const [sectionData, setSectionData] = useState(sections?.["section-2"] || {
    title: "",
    description: "",
    link: { name: "", url: "" },
    images: [],
  ...
  useEffect(() => {
    if (sections?.["section-2"]) {
      setSectionData(sections["section-2"])
    }
  }, [sections])

  const handleSave = () => {
    updateSection("section-2", sectionData)
          ...
          <div className="grid grid-cols-2 gap-4">
            {sectionData.images_block?.map((image, index) => (
              <div key={index} className="space-y-2">
                <Image
              width={300}
              height={300} src={image.src || "/placeholder.svg"} alt={image.alt} className="w-full h-40 object-cover" />
